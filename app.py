...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("**Dataset:** HR Analytics – Attrition & Risk Levels")
...
if uploaded is None:
    st.info("👆 Upload `hr_employee_attrition_data.csv` from the sidebar to get started.")
    st.stop()
...
def load(file, ts, rs):
    ...
    if df["Attrition_Risk_Level"].dtype == object:
        df["Attrition_Risk_Level"] = df["Attrition_Risk_Level"].map({"Low": 0, "Medium": 1, "High": 2})
...
df, X_train, X_test, y_train, y_test = load(uploaded, test_size, random_state)
...
with T[2]:
    ...
    cols_or = [RED if v >= 1 else ACCENT for v in or_df["Odds Ratio"]]
    ax.barh(or_df.index, or_df["Odds Ratio"], color=cols_or, edgecolor="none")
...
with T[3]:
    ...
    sub = st.tabs(["Model 1 — With Confounding Var", "Model 2 — Without Confounding Var",
    ...
    with sub[2]:
        ...
        vif_df = pd.DataFrame({"Variable": LR_FEATURES,
        ...
        st.markdown(insight(
            ...
            "All VIF values are below 10, so no severe multicollinearity is present."
        ), unsafe_allow_html=True)
...
with T[4]:
    ...
    with cb:
        ...
        fig, ax = dark_fig(5.5, 4.5)
        ax.plot(fpr_lda, tpr_lda, color=ACCENT, linewidth=2,
                label=f"AUC = {auc_lda:.3f}")
        ax.plot([0,1],[0,1],"--", color="#555", linewidth=1)
    ...
    with cd:
        ...
        ax.axvline(best["Threshold"], color="white", linestyle="--", linewidth=1, alpha=0.6, label=f'Best = {best["Threshold"]}')
...
with T[5]:
    ...
    rep_qda = classification_report(y_test, y_pred_qda, target_names=["Low","Medium","High"], output_dict=True)
...
with T[7]:
    ...
    rows = []
    configs = [
        ("Binary LR (High vs Others)", y_pred_bin,  y_test_binary, True),
        ("MNLogit — Model 1 (with Confound)", y_pred_mc1, y_test, False),
        ("MNLogit — Model 2 (no Confound)",  y_pred_mc2, y_test, False),
        ("LDA",         y_pred_lda, y_test, False),
        ("QDA",         y_pred_qda, y_test, False),
        ("Naive Bayes", y_pred_nb,  y_test, False),
    ]
    for name, yp, yt, binary in configs:
        rows.append({
            "Model":     name,
...
with T[8]:
    ...
    if "Num_Projects" not in df.columns:
        ...
        axes[0].scatter(y_reg.iloc[idx], y_pred_lin.iloc[idx],
                        alpha=0.25, color=ACCENT, s=12)
        axes[0].plot([y_reg.min(), y_reg.max()], [y_reg.min(), y_reg.max()],
                     "r--", linewidth=1)
        ...
        axes[1].scatter(y_reg.iloc[idx], y_pred_poi.iloc[idx],
                        alpha=0.25, color=AMBER, s=12)
        axes[1].plot([y_reg.min(), y_reg.max()], [y_reg.min(), y_reg.max()],
                     "r--", linewidth=1)
        ...
        st.pyplot(fig); plt.close()
        st.markdown(insight(
            "📌 Predicted values cluster around the mean (~5), while actuals range 2–8. "
            "Both models fail to capture meaningful variation — confirming weak fit."
        ), unsafe_allow_html=True)
        ...
        ax.set_xlabel("Num_Projects", color="white")
        ax.set_ylabel("Frequency",   color="white")
        ax.set_title("Distribution: Actual vs Predicted", color="white")
        ax.legend(facecolor=DARK2, labelcolor="white")
        st.pyplot(fig); plt.close()
        st.markdown(insight(
            "📌 Both models produce predictions concentrated around the mean, while actual "
            "values show wider spread — indicating limited explanatory power of the chosen predictors."
        ), unsafe_allow_html=True)
        ...
        fig, ax = dark_fig(9, 4)
        ax.scatter(y_pred_lin.iloc[idx], resid.iloc[idx],
                   alpha=0.25, color=PURPLE, s=12)
        ...
        st.pyplot(fig); plt.close()
        st.markdown(insight(
            "📌 Residuals show a <b>structured pattern</b> rather than random scatter, "
            "indicating that the linear regression assumptions may not fully hold for count data "
            "— Poisson is theoretically more appropriate, but neither model achieves meaningful fit."
        ), unsafe_allow_html=True)
